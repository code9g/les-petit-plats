...
const minLength = 2;
const text = "ail";
const words = text.length > minLength ? text.split(/s+/) : [];

function test1(recipes) {
  const reList = [];
...
console.log(`Préparation de ${m * recipes.length} recette(s)`);

for (let i = m; i > 0; i--) {
  testRecipes = [...testRecipes, ...recipes];
}

console.log("Démarrage du test");

let t1 = performance.now();
for (let i = n; i > 0; i--) test1(testRecipes);
t1 = performance.now() - t1;

let t2 = performance.now();
for (let i = n; i > 0; i--) test2(testRecipes);
t2 = performance.now() - t2;

console.log("Native       :", `${Math.round((1000 * n) / t1)} fn/s`);
console.log("Fonctionnelle:", `${Math.round((1000 * n) / t2)} fn/s`);
